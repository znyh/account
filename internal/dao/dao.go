...
type Dao interface {
	...
	business //业务流接口(account)
}

type business interface {
	GuestRegister(ctx context.Context, u *pb.UserInfo, m map[string]interface{}) (err error)
	ApplySN(ctx context.Context) (sn int32, err error)
	DisBindSN(ctx context.Context) (err error)
}
